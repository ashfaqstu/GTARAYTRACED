  ...
  for(x=sx-scale/2;x<sx+scale/2;x++)
  {
   ...
   for(y=0;y<scale;y++)
   {
    if(sp[s].state==1 && x>0 && x<120 && b<depth[x])
    {
     ...
     {
     iPoint(8,x*8,sy*8-y*8,red,green,blue);
...
//    glPointSize(8); glColor3ub(red,green,blue); glBegin(GL_POINTS); glVertex2i(r*8,y+lineOff); glEnd();
   iPoint(8,r*8,y+lineOff,red,green,blue);
   ty+=ty_step;
  }
...
//   glPointSize(8); glColor3ub(red,green,blue); glBegin(GL_POINTS); glVertex2i(r*8,y); glEnd();
 ...
 //---draw ceiling---
  ...
  if(mp>0){ 
	...
	iPoint(8,r*8,640-y,red,green,blue);
	}
 }
...
{int x,y;
 for(y=0;y<40;y++)
 {
  ...
  // glPointSize(8); glColor3ub(red,green,blue); glBegin(GL_POINTS); glVertex2i(x*8,y*8); glEnd();
   iPoint(8,x*8,y*8,red,green,blue);
  }	
 }
}
...
void iKeyboardDown(unsigned char key) {
	if(key=='a'){ Keys.a=1;} 	
 if(key=='d'){ Keys.d=1;} 
 if(key=='w'){ Keys.w=1;}
 if(key=='s'){ Keys.s=1;}
